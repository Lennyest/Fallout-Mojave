...
public class PawnMovement : EntityComponent<Pawn>
{
	...
	public void Simulate( IClient cl )
	{
		...
		if (Entity.Noclipping)
		{
			Entity.Position = Entity.Position + Rotation.From( angles ) * movement * 20f;

			if ( Input.Down( InputButton.Jump ) )
			{
				Entity.Position += Vector3.Up * 10f;
			} 
			else if ( Input.Down( InputButton.Duck) )
			{
				Entity.Position -= Vector3.Up * 5f;
			}
		...
		if ( groundEntity.IsValid() )
		{
			...
			var velocity = 200.0f * (Input.Down( InputButton.Run ) ? 0.5f : 1.0f);

			if (Input.Down(InputButton.Back) && !(Input.Down( InputButton.Run )))
				velocity = 200.0f * 0.4f;

			Entity.Velocity = Accelerate( Entity.Velocity, moveVector.Normal, moveVector.Length, velocity, 7.5f );
